...
void insert(Nodes::Header** node_header_ptr /* pointer to the parent's pointer to the child */, Key key, Value value, size_t depth) {
    ...
    size_t first_diff;
    for (first_diff = 0; 
         first_diff < (*node_header_ptr)->prefix_len &&
           first_diff < key.length;
         ++first_diff) {
        if (key[depth + first_diff] == (*node_header_ptr)->prefix[first_diff]) break;
    }
    ...
    if (first_diff != (*node_header_ptr)->prefix_len) {
        ...
        new_node_header->prefix = (uint8_t*) malloc(first_diff * sizeof(uint8_t)); // TODO: arena
        ...
        memmove((*node_header_ptr)->prefix, (*node_header_ptr)->prefix + first_diff, (*node_header_ptr)->prefix_len);
    ...
    // Pointer to the location in memory where the child was found. May be header, may be leaf
    void** next_src = Nodes::findChild(*node_header_ptr, key[depth]);
    ...
    if (Nodes::isLeaf(*next_src)) {
        ...
        size_t i = depth;
        while (i - depth < Nodes::PREFIX_SIZE &&
               i < key.length &&
               i < leaf->key.length &&
               key[i] == leaf->key[i]) {
            ++i;
        ...
        new_node_header->prefix = (uint8_t*) malloc(new_node_header->prefix_len * sizeof(uint8_t)); // TODO: arena
        new_node_header->children_count = 2;
        memcpy(new_node_header->prefix, leaf->key.data + depth, new_node_header->prefix_len);
