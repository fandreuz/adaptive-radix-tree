...
#define DISPATCH_CHILDREN_COUNT(action, nt)   \
    if (nt == Type::NODE4)   action(4);   \
    if (nt == Type::NODE16)  action(16);  \
    if (nt == Type::NODE48)  action(48);  \
    if (nt == Type::NODE256) action(256); \
    ShouldNotReachHere
    ...
    if ((*node_header)->type == Type::NODE4) {
        ...
        for (uint8_t i = 0; i < (*node_header)->children_count; ++i) {
            if (node->children[i] != nullptr) {
                ...
                if (child_index == (*node_header)->children_count) break;
            }
        }
...
// TODO: Improve
void addChildSmallNode(uint8_t* keys, void** children, uint8_t count, Key key, Value value, size_t depth) {
    uint8_t i = 0;
    while(children[i] != nullptr && keys[i] < key[depth]) ++i;
...
// TODO: Improve
void** findChildSmallNode(uint8_t* keys, void** children, uint8_t key, uint8_t count) {
...
void** findChild(Header* node_header, uint8_t key) {
    if (node_header->type == Type::NODE4) {
        ...
        if (child_index == Node48::EMPTY) return nullptr;
...
void printAndGo(char c, void* next, std::ostream& os, size_t depth) {
    ...
    if (isLeaf(next)) {
        Leaf* l = asLeaf(next);
        os << " --(" << l->key << ")--> " << l->value << std::endl;
        return;
    }
    os << std::endl;
    print((Header*) next, os, depth + 1);
}

void printSmallNode(uint8_t* keys, void** children, uint8_t count, std::ostream& os, size_t depth) {
    for (uint8_t i = 0; i < count; ++i) {
        if (children[i] != nullptr) {
            printAndGo((char) keys[i], children[i], os, depth);
        }
    }
}

void print(Header* node_header, std::ostream& os, size_t depth) {
    if (node_header->type == Type::NODE4) {
        auto node = (Node4*) node_header->getNode();
        printSmallNode(node->keys, node->children, 4, os, depth);
        return;
    } else if (node_header->type == Type::NODE16) {
        auto node = (Node16*) node_header->getNode();
        printSmallNode(node->keys, node->children, 16, os, depth);
        return;
    } else if (node_header->type == Type::NODE48) {
        auto node = (Node48*) node_header->getNode();
        for (uint8_t i = 0; i < 256; ++i) {
            if (node->child_index[i] != Node48::EMPTY) {
                printAndGo((char) i, node->children[node->child_index[i]], os, depth);
            }
        }
        return;
    } else if (node_header->type == Type::NODE256) {
        ...
        for (uint8_t i = 0; i < 256; ++i) {
            if (node->children[i] != nullptr) {
                printAndGo((char) i, node->children[i], os, depth);
            }
        }
        return;
    }
